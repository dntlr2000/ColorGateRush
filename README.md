...
The app opens on a tap-to-start title screen, then enters MainMenu after explicit player input.
...
Gameplay visual assets are generated from Unity primitives, built-in UI/TextMesh, ParticleSystem, and procedural materials. The app opens on the approved tap-to-start title image at `Assets/_Project/Resources/ColorGateRush/Images/TitleScreen.png`; tapping/clicking it enters the runtime MainMenu, which keeps its own approved background image at `Assets/_Project/Resources/ColorGateRush/Images/MainMenuBackground.png`. A separate custom splash scene/timer is not used. The Android app icon is stored at `Assets/_Project/Art/AppIcon/color_gate_rush_icon_1024.png` and Bootstrap Project reapplies it to Player Settings when Unity's icon API is available. Runtime gameplay visuals still use procedural geometry. Audio uses two approved user-provided BGM clips for menu/gameplay music, with procedural `AudioClip.Create` fallbacks, SFX, and result stings.
...
- BGM source/license records are tracked in `docs/audio_licenses.md`; both BGM files are recorded as Gemini-generated by user, and the Gemini terms/commercial-use TODO must be resolved before public store submission.
...
For Android/WebGL packaging preparation, follow `docs/release_readiness_checklist.md`. It covers Validate Build, Balance Report, Release Readiness Report, APK/AAB usage, target API/manual signing checks, Gemini BGM license records, Android icon checks, WebGL browser checks, and device smoke tests.
...
For store-copy preparation, use `docs/store_listing_draft.md`. It includes draft descriptions, screenshot candidates, privacy/data notes, BGM license reminders, and release TODOs.

## Portfolio Summary

Color Gate Rush is a Unity 6 mobile portrait runner built as a release-candidate solo/portfolio project. It demonstrates procedural stage generation, deterministic balance validation, runtime-safe primitive rendering, URP-compatible material provisioning, Korean/English localization, mobile-first HUD/settings UI, BGM/SFX settings, and Android release preparation without Asset Store dependencies.

Release candidate target:

- Google Play internal testing artifact: signed AAB
- Package ID: `com.nappa.colorgaterush`
- Version: `0.9.0`
- Android Version Code: `1`
- Data model: local-only `PlayerPrefs`, no analytics/ad/network SDKs

Portfolio material TODO:

- Capture final Android device screenshots: Title Screen, Main Menu, Stage gameplay, Endless gameplay, Stage Select, Result, and Settings.
- Record a short portrait gameplay clip showing Stage Mode, Endless Mode, language switching, and settings volume controls.
- Confirm Gemini-generated BGM terms before public portfolio/store distribution claims.
- Keep keystore files, passwords, and Play Console credentials outside the repository.
