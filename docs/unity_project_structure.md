...
- `PlaytestStats.cs` records local-only stage attempts, clears, fails, quits, scores, stars, and playtime under `CGR_Stats_` keys for manual playtest review.
- `EndlessRunConfig.cs` and `EndlessRecords.cs` define the finish-free Endless MVP tuning and `CGR_Endless...` best record storage.
...
- Endless Mode is launched from Main Menu, is independent from stage stars/unlocks, and cleans rolling generated chunks behind the player.
